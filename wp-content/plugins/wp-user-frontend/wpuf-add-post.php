...
class WPUF_Add_Post {
    ...
    function shortcode( $atts ) {
        ...
        ob_start();
        ?>
        <div style="    text-align: center;    font-style: italic;    font-weight: bold;  margin-bottom: 10px;"> 
                 ~: Send Best Wishes to Your Lovable Person :~ </div>
        <?php
...
<a href="<?php echo site_url()?>/sendstatus"> Wishes Status</a></div>
<?php
        $userdata = get_user_by( 'id', $userdata->ID );
            if($_REQUEST['pid']){
                 echo '<div class="success">' . __('Post published successfully') . '</div>';
    ...
    function publish_date_form() {
        ...
        <li>
            <label for="timestamp-wrap">
                <?php _e( 'Time:', 'wpuf' ); ?> <span class="required">*</span>
            </label>
    ...
    function submit_post() {
        ...
        if ( $post_date_enable == 'on' ) {
            ...
            $dob=date( 'Y-m-d H:i:s', $post_date );
        }
        ...
        if ( $post_id ) {
            ...
            if ( $custom_fields ) {
                foreach ($custom_fields as $key => $val) {
                    ...
                    add_post_meta( $post_id, 'date-of-birthday', $dob, true );
            ...
            if ( $post_id ) {
                ...
                wp_redirect( site_url('/postingads/?pid='.$post_id) );
           }
        }
    }
